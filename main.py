...
@application.route('/create_wallet', methods=['GET'])
def create_wallet():
...
@application.route('/get_balance_bnb/<address>', methods=['GET'])
def get_balance_bnb(address):
    balance = w3.eth.get_balance(address)
    return jsonify({'balance': balance*10**-18})


@application.route('/get_balance_usdt/<address>', methods=['GET'])
...
@application.route('/transfer_bnb', methods=['POST'])
def transfer_bnb():
    data = request.json
...
@application.route('/transfer_usdt', methods=['POST'])
def transfer_usdt():
        data = request.json
        from_private_key = data['from_private_key']
        from_address = data['from_address']
...
@application.route('/increase_allowance', methods=['POST'])
